...
void init_kernel_memory(uint32_t* kernel_end)
{
    ...
    memset2((char *)heap_begin, 0, heap_end - heap_begin);
    pheap_desc = (uint8_t *)malloc(MAX_PAGE_ALIGNED_ALLOCS);
    printt("Kernel heap starts at 0x%x\n", last_alloc);
}

// Print the current memory layout
void print_memory_layout()
{
    printt("Memory used: %d bytes\n", memory_used);
    printt("Memory free: %d bytes\n", heap_end - heap_begin - memory_used);
    printt("Heap size: %d bytes\n", heap_end - heap_begin);
    printt("Heap start: 0x%x\n", heap_begin);
    printt("Heap end: 0x%x\n", heap_end);
    printt("PHeap start: 0x%x\nPHeap end: 0x%x\n", pheap_begin, pheap_end);
}
...
char* pmalloc(size_t size)
{
    ...
    for(int i = 0; i < MAX_PAGE_ALIGNED_ALLOCS; i++)
    {
        ...
        pheap_desc[i] = 1;
        printt("PAllocated from 0x%x to 0x%x\n", pheap_begin + i*4096, pheap_begin + (i+1)*4096);
        return (char *)(pheap_begin + i*4096);
    }
    printt("pmalloc: FATAL: failure!\n");
    return 0;
}
...
void* malloc(size_t size)
{
    ...
    while((uint32_t)mem < last_alloc)
    {
        ...
        printt("mem=0x%x a={.status=%d, .size=%d}\n", mem, a->status, a->size);
        ...
        {
            a->status = 1;
            printt("RE:Allocated %d bytes from 0x%x to 0x%x\n", size, mem + sizeof(alloc_t), mem + sizeof(alloc_t) + size);
            memset2(mem + sizeof(alloc_t), 0, size);
    ...
    {
        printt("Cannot allocate bytes! Out of memory.\n");
    }
    ...
    last_alloc += 4;
    printt("Allocated %d bytes from 0x%x to 0x%x\n", size, (uint32_t)alloc + sizeof(alloc_t), last_alloc);
    memory_used += size + 4 + sizeof(alloc_t);
    memset2((char *)((uint32_t)alloc + sizeof(alloc_t)), 0, size);
