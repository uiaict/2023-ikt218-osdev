...
void ESOS::IDT::init_idt()
{
  ...
  for (int i = 0; i < IDT_ENTRIES; i++)
  {
    ...
    ESOS::IDT::int_handlers[i].handler = NULL;
  }

  ESOS::IDT::init_interrupts();
  ESOS::IDT::init_interrupt_handlers();
  ESOS::IDT::init_irq();
  ESOS::IDT::init_irq_handlers();

  // Load the IDT
  idt_flush((uint32_t)&idt_ptr);
}

void ESOS::IDT::idt_set_gate(uint8_t num, uint32_t base, uint16_t sel, uint8_t flags)
{
