...
typedef struct idt_entry {
	...
	uint16_t    base_high;    // The GDT segment selector that the CPU will load into CS before calling the ISR
	uint8_t     selector;     // Set to zero
	uint8_t     zero;   // Type and attributes; see the IDT page
	uint16_t    flags;     // The higher 16 bits of the ISR's address
