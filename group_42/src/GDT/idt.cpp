...
void init_idt() // function called from assambly to inizilise the interupt descriptor table
{
   ...
   // sends a byte to ports
    ...
    outb(0x21, 0x0);
    outb(0xA1, 0x0);
    // sets up the entries/gates in the IDT
...
void idt_set_gate(uint8_t num, uint32_t base, uint16_t sel, uint8_t flags) // sets up a gate into the idt
{
    idt_entries[num].base_lo = base & 0xFFFF;// memory address
    idt_entries[num].base_hi = (base >> 16) & 0xFFFF;  // memory adress

    idt_entries[num].sel     = sel; //GDT offset of memory segment the code is in
