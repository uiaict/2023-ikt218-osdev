...
void init_kernel_memory(uint32_t* kernel_end)
{
    ...
    printf("Kernel heap starts at 0x%i\n", last_alloc);
}

// Print the current memory layout
void print_memory_layout()
{
    printf("Memory used: %i bytes\n", memory_used);
    printf("Memory free: %i bytes\n", heap_end - heap_begin - memory_used);
    printf("Heap size: %i bytes\n", heap_end - heap_begin);
    printf("Heap start: 0x%i\n", heap_begin);
    printf("Heap end: 0x%i\n", heap_end);
    printf("PHeap start: 0x%i\nPHeap end: 0x%i\n", pheap_begin, pheap_end);
}
...
void* malloc(size_t size)
{
    ...
    while((uint32_t)mem < last_alloc)
    {
        ...
        printf("mem=0x%i a={.status=%i, .size=%i}\n", mem, a->status, a->size);
        ...
        {
            a->status = 1;
            printf("RE:Allocated %i bytes from 0x%i to 0x%i\n", size, mem + sizeof(alloc_t), mem + sizeof(alloc_t) + size);
