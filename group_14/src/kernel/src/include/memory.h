...
extern char* custom_pmalloc(size_t size); /* Allocates memory of given size with page alignment */
extern void* custom_malloc(size_t size); /* Allocates memory of given size */
extern void custom_free(void *mem); /* Frees memory previously allocated */

/* Function declarations for memory manipulation */
extern void* custom_memcpy(void* dest, const void* src, size_t num ); /* Copies num bytes from src to dest */
extern void* custom_memset (void * ptr, int value, size_t num ); /* Sets num bytes starting from ptr to value */
extern void* custom_memset16 (void *ptr, uint16_t value, size_t num); /* Sets num bytes starting from ptr to a 16-bit value */
