...
void init_kernel_memory(uint32_t* kernel_end)
{
    ...
    custom_memset((char *)heap_begin, 0, heap_end - heap_begin);
    pheap_desc = (uint8_t *)custom_malloc(MAX_PAGE_ALIGNED_ALLOCS);
...
char* custom_pmalloc(size_t size)
{
...
void* custom_malloc(size_t size)
{
    ...
    while((uint32_t)mem < last_alloc)
    {
        ...
        if(a->size >= size)
        {
            ...
            custom_memset(mem + sizeof(alloc_t), 0, size);
    ...
    custom_memset((char *)((uint32_t)alloc + sizeof(alloc_t)), 0, size);
